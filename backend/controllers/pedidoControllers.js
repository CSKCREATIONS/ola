...
exports.enviarPedidoAgendadoPorCorreo = async (req, res) => {
  try {
    ...
    console.log('🔍 Enviando pedido agendado por correo:', pedidoId);
    ...
    const asuntoFinal = asunto || `Pedido Agendado N° ${pedido.numeroPedido} - ${process.env.COMPANY_NAME || 'JLA Global Company'}`;

    // Generar PDF del pedido
    let pdfAttachment = null;
    try {
      console.log('🔍 [AGENDADO] Intentando generar PDF para pedido:', pedido.numeroPedido);
      console.log('🔍 [AGENDADO] Productos en pedido:', pedido.productos?.length || 0);
      const pdfService = new PDFService();
      console.log('✅ [AGENDADO] PDFService instanciado');
      const pdfData = await pdfService.generarPDFPedido(pedido, 'agendado');
      console.log('✅ [AGENDADO] PDF generado:', pdfData.filename, 'Tamaño:', pdfData.buffer?.length, 'bytes');
      pdfAttachment = {
        filename: pdfData.filename,
        content: pdfData.buffer,
        contentType: pdfData.contentType
      };
      console.log('✅ [AGENDADO] PDF adjunto preparado');
    } catch (pdfError) {
      console.error('❌ [AGENDADO] Error completo al generar PDF:', pdfError);
      console.error('❌ [AGENDADO] Stack:', pdfError.stack);
      console.warn('⚠️ [AGENDADO] Continuando sin PDF...');
    }

    console.log('📧 [AGENDADO] Enviando correo con PDF:', !!pdfAttachment);
    await enviarCorreoConAttachment(correoDestino, asuntoFinal, htmlContent, pdfAttachment);

    return res.status(200).json({
      success: true,
      message: '¡Pedido agendado enviado por correo exitosamente!' + (pdfAttachment ? ' con PDF adjunto' : ''),
      details: {
    ...
    console.error('❌ Error al enviar pedido agendado por correo:', error);
...
// Enviar pedido por correo (general - para cualquier tipo de pedido)
exports.enviarPedidoPorCorreo = async (req, res) => {
  console.log('=====================================================');
  console.log('📧 [INICIO] enviarPedidoPorCorreo llamado');
  console.log('📧 [INICIO] Params:', req.params);
  console.log('📧 [INICIO] Body:', req.body);
  console.log('=====================================================');
  ...
  try {
    ...
    console.log('🔍 [CHECK] pedidoId sanitizado:', pedidoId);
    ...
    if (!correoDestino) {
      console.log('❌ [ERROR] Correo de destino no proporcionado');
      return res.status(400).json({ success: false, message: 'El correo de destino es requerido' });
    }

    console.log('🔍 Enviando pedido por correo:', pedidoId);
    ...
    const htmlContent = generarHTMLPedidoAgendado(pedido, mensaje || '');
    const asuntoFinal = asunto || `Pedido N° ${pedido.numeroPedido} - ${process.env.COMPANY_NAME || 'JLA Global Company'}`;

    // Generar PDF del pedido
    let pdfAttachment = null;
    try {
      console.log('🔍 [GENERAL] Intentando generar PDF para pedido:', pedido.numeroPedido);
      console.log('🔍 [GENERAL] Productos en pedido:', pedido.productos?.length || 0);
      const pdfService = new PDFService();
      console.log('✅ [GENERAL] PDFService instanciado');
      const pdfData = await pdfService.generarPDFPedido(pedido, 'agendado');
      console.log('✅ [GENERAL] PDF generado:', pdfData.filename, 'Tamaño:', pdfData.buffer?.length, 'bytes');
      pdfAttachment = {
        filename: pdfData.filename,
        content: pdfData.buffer,
        contentType: pdfData.contentType
      };
      console.log('✅ [GENERAL] PDF adjunto preparado');
    } catch (pdfError) {
      console.error('❌ [GENERAL] Error completo al generar PDF:', pdfError);
      console.error('❌ [GENERAL] Stack:', pdfError.stack);
      console.warn('⚠️ [GENERAL] Continuando sin PDF...');
    }

    console.log('📧 [GENERAL] Enviando correo con PDF:', !!pdfAttachment);
    await enviarCorreoConAttachment(correoDestino, asuntoFinal, htmlContent, pdfAttachment);

    return res.status(200).json({
      success: true,
      message: '¡Pedido enviado por correo exitosamente!' + (pdfAttachment ? ' con PDF adjunto' : ''),
      details: {
        destinatario: correoDestino,
        asunto: asuntoFinal,
        numeroPedido: pedido.numeroPedido,
...
exports.enviarPedidoCanceladoPorCorreo = async (req, res) => {
  try {
    ...
    console.log('🔍 Enviando notificación de pedido cancelado:', pedidoId);
    ...
    const htmlContent = generarHTMLPedidoCancelado(pedido, mensaje || '', motivoCancelacion || 'No especificado');
    const asuntoFinal = asunto || `Pedido Cancelado N° ${pedido.numeroPedido} - ${process.env.COMPANY_NAME || 'JLA Global Company'}`;

    await enviarCorreoConAttachment(correoDestino, asuntoFinal, htmlContent, null);

    return res.status(200).json({
      success: true,
      message: '¡Notificación de pedido cancelado enviada exitosamente!',
      details: {
        destinatario: correoDestino,
        asunto: asuntoFinal,
        numeroPedido: pedido.numeroPedido
    ...
    console.error('❌ Error al enviar notificación de pedido cancelado:', error);
    return res.status(500).json({
      success: false,
      message: 'Error al enviar correo',
      error: error.message
...
// Test de configuración de correo
exports.testEmailConfiguration = async (req, res) => {
  try {
    const testEmail = req.body.email || process.env.GMAIL_USER || 'test@example.com';
    ...
    const htmlContent = `
      ...
      <body style="font-family: Arial, sans-serif; padding: 20px;">
        <h2 style="color: #fd7e14;">✅ Configuración de Correo Funcionando</h2>
        <p>Este es un correo de prueba del sistema de gestión de pedidos.</p>
        <p><strong>Fecha:</strong> ${new Date().toLocaleString('es-ES')}</p>
        <p><strong>Sistema:</strong> ${process.env.COMPANY_NAME || 'JLA Global Company'}</p>
      </body>
      </html>
    `;

    await enviarCorreoConAttachment(
      testEmail,
      'Test de Configuración de Correo - Sistema de Gestión',
      htmlContent,
      null
    );
    ...
    return res.status(200).json({
      ...
      details: { destinatario: testEmail }
    });
  } catch (error) {
...
exports.deletePedidocancelado = async (req, res) => {
  try {
    ...
    if (pedido.estado !== 'cancelado' && pedido.estado !== 'Cancelado') {
      return res.status(400).json({
        success: false,
    ...
    return res.status(200).json({
      success: true,
      message: 'Pedido cancelado eliminado exitosamente',
      pedidoId
    });
  } catch (error) {
